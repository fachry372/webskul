...
<div class="w-full bg-white rounded-lg shadow my-8 px-4 sm:px-6 md:px-12 lg:px-20 py-6" data-aos="fade-up">
    @forelse($menus as $menu)
        @php
            $hasContent = !empty($menu->content)
                || !empty($menu->content_section_photos)
                || !empty($menu->content_section_files);
        ...
        <div class="mb-6" data-aos="fade-up">
            ...
            @if(!empty($photos) && is_array($photos))
                ...
                <div class="grid mt-4 gap-4
                    ...
                    @if(!empty($photosText))
                        <div class="content-html col-span-full mt-4">{!! $photosText !!}</div>
                    @endif
                </div>
            @endif
            ...
            @if(!empty($files) && is_array($files))
                <div class="mt-4 grid gap-6">
                    @foreach($files as $file)
                        @php
                            $path = is_array($file) && isset($file['path']) ? $file['path'] : $file;
                        ...
                        @else
                            <div class="file-box text-center">
                                <p class="mb-2 text-gray-600">Preview tidak tersedia untuk tipe ini.</p>
        ...
        @endif
    @empty
        <p class="text-xl text-red-600 font-semibold text-center">
            Belum ada data untuk profil ini.
        </p>
    @endforelse
</div>
@endsection
