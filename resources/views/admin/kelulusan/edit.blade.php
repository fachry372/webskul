...
<div class="max-w-6xl mx-auto bg-white p-6 rounded-lg shadow">
    <h2 class="text-2xl font-bold mb-4">Edit Kelulusan</h2>

    @if(session('success'))
        <p class="text-green-600 mb-4">{{ session('success') }}</p>
    @endif

    <form id="kelulusanEditForm" action="{{ route('admin.kelulusan.update', $kelulusan->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')

        <div class="mb-4">
            <label for="judul" class="block font-semibold">Judul Kelulusan</label>
            <input type="text" name="judul" id="judul"
                   class="w-full border rounded p-2 @error('judul') border-red-500 @enderror"
                   value="{{ old('judul', $kelulusan->judul) }}" required>
            @error('judul')
...
<script>
// Sama persis seperti edit Galeri, tapi diganti id form dan variable kelulusan
