...
@section('title', 'Edit Konten Lainnya')
...
<div class="max-w-6xl mx-auto bg-white p-6 rounded-lg shadow">
    <h2 class="text-2xl font-bold mb-4">Edit Konten Lainnya</h2>

    @if(session('success'))
        <p class="text-green-600 mb-4">{{ session('success') }}</p>
    @endif

    <form id="lainnyaEditForm" action="{{ route('admin.lainnya_konten.update', $lainnya_konten->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')

        {{-- Pilih Menu Lainnya --}}
        <div class="mb-4">
            <label for="lainnya_id" class="block font-semibold">Pilih Menu Lainnya</label>
            <select name="lainnya_id" id="lainnya_id" class="w-full border rounded p-2 @error('lainnya_id') border-red-500 @enderror" required>
                <option value="">-- Pilih Menu --</option>
                @foreach($lainnyas as $lainnya)
                    <option value="{{ $lainnya->id }}" {{ old('lainnya_id', $lainnya_konten->lainnya_id) == $lainnya->id ? 'selected' : '' }}>{{ $lainnya->title }}</option>
                @endforeach
            </select>
            @error('lainnya_id')
        ...
        <div>
            <button type="submit" id="btnSubmit" class="bg-blue-600 text-white px-6 py-2 rounded hover:bg-blue-700">Simpan</button>
            <a href="{{ route('admin.lainnya_konten.index') }}" class="bg-gray-500 text-white px-6 py-2 rounded hover:bg-gray-600 ml-2">Batal</a>
        </div>
    </form>
</div>
@endsection
...
function createBlock(index, data=null){
    ...
    // Preview files lama & baru sama seperti IKM
    ['photos','videos','files'].forEach(type=>{
        ...
        if(data && data[type]){
            data[type].forEach(f=>{
                ...
                btn.onclick=()=>{ wrapper.dataset.deleted=true; const delInput=document.createElement('input'); delInput.type='hidden'; delInput.name=`blocks[${index}][_delete_files][]`; delInput.value=f; card.appendChild(delInput); wrapper.remove(); };
                wrapper.appendChild(btn); wrapper.dataset.filePath=f;
        ...
        function renderPreview(){
            ...
            filesArr.forEach((file, idx)=>{
                ...
                const btn=document.createElement('span'); btn.innerHTML='✖'; btn.title='Hapus file ini'; btn.onclick=()=>{ filesArr.splice(idx,1); renderPreview(); updateInputFiles(); };
                wrapper.appendChild(btn);
                previewElem.appendChild(wrapper);
            });
        }
        function updateInputFiles(){ const dt = new DataTransfer(); filesArr.forEach(f=>dt.items.add(f)); inputElem.files = dt.files; }
        inputElem.addEventListener('change', e=>{ filesArr = filesArr.concat(Array.from(e.target.files)); renderPreview(); updateInputFiles(); });
    });

    // Link video lama & baru
    const linksContainer=document.getElementById(`links-container-${index}`);
    function addLinkInput(val=''){ const div=document.createElement('div'); div.className='link-input'; div.innerHTML=`<input type="url" name="blocks[${index}][videos_link][]" placeholder="https://example.com" value="${val ?? ''}"><button type="button">✖</button>`; div.querySelector('button').addEventListener('click', ()=>div.remove()); linksContainer.appendChild(div); }
    if(data && Array.isArray(data.videos_link) && data.videos_link.length){ data.videos_link.forEach(link=>addLinkInput(link)); } else { addLinkInput(); }
    card.querySelector('.add-link').addEventListener('click', ()=>addLinkInput());
}

// Render blok lama
@if($lainnya_konten->blocks)
    @foreach($lainnya_konten->blocks as $b)
        createBlock(blockIndex++, @json($b));
    @endforeach
@endif

document.getElementById('add-block').addEventListener('click', ()=>createBlock(blockIndex++));
document.getElementById('lainnyaEditForm').addEventListener('submit', ()=>{
    document.querySelectorAll('#blocks-container .card').forEach(card=>{
        ...
        ['photos','videos','files'].forEach(type=>{
            ...
            previewElem.querySelectorAll('.old-file').forEach(el=>{ if(el.dataset.deleted) el.remove(); });
        });

        const linksContainer = document.getElementById(`links-container-${idx}`);
        linksContainer.querySelectorAll('input').forEach(input=>{ if(!input.value) input.value=''; });
    });
