...
@section('title', 'Tambah Konten Lainnya')
...
<div class="max-w-6xl mx-auto bg-white p-6 rounded-lg shadow">
    <h2 class="text-2xl font-bold mb-4">Tambah Konten Lainnya</h2>

    @if(session('success'))
        <p class="text-green-600 mb-4">{{ session('success') }}</p>
    @endif

    <form id="lainnyaCreateForm" action="{{ route('admin.lainnya_konten.store') }}" method="POST" enctype="multipart/form-data">
        @csrf
        ...
        <div class="mb-4">
            ...
            <select name="lainnya_id" id="lainnya_id" class="w-full border rounded p-2 @error('lainnya_id') border-red-500 @enderror" required>
                ...
                @foreach($lainnyas as $lainnya)
                    <option value="{{ $lainnya->id }}" {{ old('lainnya_id') == $lainnya->id ? 'selected' : '' }}>{{ $lainnya->title }}</option>
                @endforeach
            </select>
            @error('lainnya_id')
...
<script>
// Script untuk form Tambah Konten Lainnya
    ...
    ['photos','videos','files'].forEach(type=>{
        ...
        function renderPreview(){
            ...
            filesArr.forEach((file, idx)=>{
                ...
                else if(type==='files'){ const fileBox=document.createElement('div'); fileBox.style.display='flex'; fileBox.style.alignItems='center'; fileBox.style.background='#f9fafb'; fileBox.style.border='1px solid #ddd'; fileBox.style.borderRadius='6px'; fileBox.style.padding='6px 10px'; fileBox.style.marginBottom='6px'; fileBox.innerHTML=`📄 <span style="margin-left:8px;">${file.name}</span>`; wrapper.appendChild(fileBox); }
        ...
        inputElem.addEventListener('change', e=>{ const newFiles = Array.from(e.target.files); filesArr = filesArr.concat(newFiles); renderPreview(); updateInputFiles(); });
    });

    const linksContainer=document.getElementById(`links-container-${index}`);
    function addLinkInput(){ const div=document.createElement('div'); div.className='link-input'; div.innerHTML=`<input type="url" name="blocks[${index}][videos_link][]" placeholder="https://example.com"><button type="button">✖</button>`; div.querySelector('button').addEventListener('click', ()=>div.remove()); linksContainer.appendChild(div); }
    addLinkInput();
...
document.getElementById('lainnyaCreateForm').addEventListener('submit', ()=>{
