...
function createBlock(index, data=null){
    ...
    // File preview & handling lama + baru
    ['photos','videos','files'].forEach(type=>{
        ...
        if(data && data[type]){
            data[type].forEach(f=>{
                ...
                else { const fileBox=document.createElement('div'); fileBox.style.cssText='display:flex;align-items:center;background:#f9fafb;border:1px solid #ddd;border-radius:6px;padding:6px 10px;margin-bottom:6px;'; fileBox.innerHTML=`📄 <span style="margin-left:8px;">${f.split('/').pop()}</span>`; wrapper.appendChild(fileBox); }
...
document.getElementById('galeriEditForm').addEventListener('submit', function () {
    document.querySelectorAll('#blocks-container .card').forEach(card=>{
        ...
        // Link video kosongkan jika tidak ada
