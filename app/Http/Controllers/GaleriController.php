        ...
        if ($request->has('blocks')) {
            foreach ($request->blocks as $index => $block) {
                ...
                if (isset($block['_delete']) && $block['_delete']) {
                    if (isset($block['id'])) {
                        GaleriBlock::find($block['id'])->delete();
                    }
                    continue;
                }

                // Ambil blok lama jika ada
                $galeriBlock = $block['id'] ? GaleriBlock::find($block['id']) : new GaleriBlock();
                $galeriBlock->galeri_id = $galeri->id;
                $galeriBlock->title = $block['title'] ?? null;
                $galeriBlock->text = $block['text'] ?? null;

                // === FOTO ===
                $oldPhotos = $block['id'] ? json_decode(GaleriBlock::find($block['id'])->photos, true) ?? [] : [];
                ...
                // === VIDEO ===
                $oldVideos = $block['id'] ? json_decode(GaleriBlock::find($block['id'])->videos, true) ?? [] : [];
                ...
                // === FILE LAIN ===
                $oldFiles = $block['id'] ? json_decode(GaleriBlock::find($block['id'])->files, true) ?? [] : [];
